...
static void hl_show_prog_info(void)
{
    ...
    /* Display SPI flash info */
    const unsigned int pages = flash_get_page_count();
    const unsigned int page_size = flash_get_page_size();
    printf("SPI (external) flash has %u pages of %u bytes each.\n", pages, page_size);

    /* If metadata is available, print the write count of highest page number */
    if (flash_supports_metadata())
    {
        ...
        printf("Page number %u has been written %u times.\n",
                highest_write_count_page, highest_write_count);
    }
}
